 ...
 * 
 *         Responsibilities of class: To test the scoring methods in the
 *         Schedule class, ensuring that points are correctly subtracted for
 *         STEM courses over three and struggle courses over one, as well as
 *         testing the break time deviation and start/end time deviation
 *         calculations. Additionally, it tests that the schedule score can be
 *         updated correctly.
 * 
 */
class ScheduleScoringTest
{
	private Schedule schedule; // A schedule scoring test has-a schedule
	...
	private Course buildCourse(String name, String id, String type,
			boolean struggle)
	{
		return new TestCourse(name, id, List.of(DayOfWeek.MONDAY),
				LocalTime.of(9, 0), LocalTime.of(10, 0), "Main", 3, 2.0, true,
				"Professor", 4.0, type, struggle);
	}
	...
	public void testSubtractPointsForSTEMCoursesOverThree()
	{
		...
		int stemCount = (int) schedule.getCurrentScheduleCourses().stream()
				.filter(c -> c.getCourseType().equals("STEM")).count();
	...
	public void testSubtractPointsForStruggleCoursesOverOne()
	{
		...
		int struggleCount = (int) schedule.getCurrentScheduleCourses().stream()
				.filter(c -> c.getStruggleCourse()).count();
	...
	private Course buildCourse(String name, String id, String type,
			boolean struggle, int startHour)
	{
		return new TestCourse(name, id, List.of(DayOfWeek.MONDAY),
				LocalTime.of(startHour, 0), LocalTime.of(startHour + 1, 0),
				"Main", 3, 2.0, true, "Professor", 4.0, type, struggle);
	}

 ...
 * 
 *         Responsibilities of class: To test the FileService class, which is
 *         responsible for handling file operations related to schedules,
 *         including saving the top three schedules to a CSV file. This test
 *         class ensures that the FileService correctly manages the top three
 *         schedules and handles edge cases such as empty schedule lists.
 * 
 */
class FileServiceTest
{
	private FileService fileService; // A file service test has-a file service
	private FakeUserDataRepository userDataRepository; // A file service test
														// has-a user data
														// repository
	...
	private static class TestCourse extends Course
	{
		public TestCourse(String name, String id, List<DayOfWeek> days,
				LocalTime start, LocalTime end, String campus, int credits,
				double difficulty, boolean required, String instructor,
				double rmp, String type, boolean struggle)
		{
			super(name, id, days, start, end, campus, credits, difficulty,
					required, instructor, rmp, type, struggle);
		}
	}
	...
	private Schedule createSchedule()
	{
		...
		Course c1 = new TestCourse("Math 101", "MATH101",
				List.of(DayOfWeek.MONDAY), LocalTime.of(9, 0),
				LocalTime.of(10, 0), "Main", 3, 2.0, true, "Prof A", 4.5,
				"STEM", false);

		Course c2 = new TestCourse("History 101", "HIST101",
				List.of(DayOfWeek.TUESDAY), LocalTime.of(10, 0),
				LocalTime.of(11, 0), "Main", 3, 1.5, false, "Prof B", 4.2,
				"Liberal Arts", false);

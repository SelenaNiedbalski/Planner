 ...
 * 
 *         Responsibilities of class: To represent the information about the
 *         student that will be used to create top schedules for them which will
 *         include their major distinction
 *         their number of courses they will input, and the courses they
 *         struggle with
 * 
 */
...
{
	// Instance Variables
	private String studentsMajorDistinction; // An about you model has-a
												// students major distinction
	private int numOfCoursesStudentWillInput; // An about you model has-a number
												// of courses the student will
												// input
	private List<String> struggleCourses; // An about you model has-many
											// struggle courses
	 ...
	 * @throws NoRadioButtonSelectedException if the students major distinction
	 *                                        is null (if no radio button is
	 *                                        selected)
	 */
	 ...
	 * (the general course that dictates number of windows, not the classes
	 * available for it)
	 * 
	 ...
	 *                                        will input
	 * @throws MustBeOverOneException if the number of courses the student will
	 *                                input is not over 1
	 */
	public void setNumOfCoursesStudentWillInput(
			int newNumOfCoursesStudentWillInput) throws MustBeOverOneException
	{
	 ...
	 *                                        with
	 * @return aboutYouErrors A hashmap of error messages for the students major
	 *         distinction and the number of courses the student will input
	 */
	public HashMap<String, String> saveAboutYou(
			String newStudentsMajorDistinction,
			int newNumOfCoursesStudentWillInput,
			List<String> newStruggleCourses)
	{
		// Create a hashmap of error messages for the students major distinction
		// and the number of courses the student will input
		HashMap<String, String> aboutYouErrors = new HashMap<String, String>();

		// Check students major distinction input
		if (newStudentsMajorDistinction == null
				|| newStudentsMajorDistinction.isEmpty())
		{
			NoRadioButtonSelectedException noRadioButtonSelectedException = new NoRadioButtonSelectedException(
					"major distinction");
			aboutYouErrors.put("studentsMajorDistinction",
		...
		if (newNumOfCoursesStudentWillInput <= 1)
		{
			MustBeOverOneException mustBeOverOneException = new MustBeOverOneException(
					"Number of courses");
			aboutYouErrors.put("numOfCoursesStudentWillInput",
	...
	// Other Methods
	/**
	 * Purpose: To clear the about you model of all information (for when the
	 * user hits back)
	 */
	public void clear()
	{

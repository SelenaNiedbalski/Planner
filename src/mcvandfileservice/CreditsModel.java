...
public class CreditsModel
{
	 ...
	 * @return creditsErrors A hashmap of error messages for the minimum and maximum required credits inputs
	 */
	public HashMap<String, String> saveCredits(int newMinRequiredCredits, int newMaxRequiredCredits)
	{
		// Create a hashmap of error messages for the minimum and maximum required credits inputs
		HashMap<String, String> creditsErrors = new HashMap<String, String>();
		
		// Check min required credits input
		if (newMinRequiredCredits <= 1)
		{
			MustBeOverOneException mustBeOverOneException = new MustBeOverOneException("Minimum required credits value");
			creditsErrors.put("minRequiredCredits", mustBeOverOneException.getMessage());
		}		
		
		// Check max required credits input
		if (newMaxRequiredCredits < newMinRequiredCredits)
		{
			EndTypeBeforeStartTypeException endTypeBeforeStartTypeException = new EndTypeBeforeStartTypeException("Max credits", "min credits");
			creditsErrors.put("maxRequiredCredits", endTypeBeforeStartTypeException.getMessage());
		}
	...
	/**
	 * Purpose: To clear the credits model of all information (for when the user hits back)
	 */
	public void clear()
	{
		minRequiredCredits = 0;

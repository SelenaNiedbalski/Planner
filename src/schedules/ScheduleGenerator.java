 ...
 * 
 *         Responsibilities of class: To represent the data model for the
 *         schedule generator which handles final calculations and creations of
...
{
	// Instance Variables
	private List<Course> allCourses; // A schedule generator model has-many
										// courses
	private List<Schedule> topThreeSchedules; // A schedule generator model
												// has-many top schedules
	private Random randomizer; // A schedule generator model has-a randomizer
	private int minCredits; // A schedule generator model has-a minimum amount
							// of credits
	private int maxCredits; // A schedule generator model has-a maximum amount
							// of credits
	private Long minBreakTime; // A schedule generator model has-a minimum break
								// time
	private Long maxBreakTime; // A schedule generator model has-a maximum break
								// time
	private HashMap<DayOfWeek, WeeklyTimeBlock> desiredStartAndEndTime; // A
																		...
																		// the
																		// week
	private String desiredCampusLocation; // A schedule generator model has-a
											// desired campus location
	private String studentsMajorDistinction; // A schedule generator model has-a
												// student's major distinction
												// (STEM, non-STEM, undecided)
	private ScheduleGeneratorRepository scheduleGeneratorRepository; // A
																		// schedule
																		// generator
																		// model
																		// has-a
																		...
																		// generator
																		// repository
	private LoadingScreenView loadingScreen; // A schedule generator model has-a
												// loading screen
	 ...
	 * 
	 * @param newLoadingScreen The loading screen to set the schedule
	 *                         generator's loading screen instance variable to
	 */
	 ...
	 * 
	 * @param newopSchedules The list of 3 schedules to set as the top 3
	 *                       schedules for testing purposes
	 */
	...
	private double calculateScheduleScore(Schedule schedule)
	{
		...
		{
			schedule.addCourseScores(desiredCampusLocation,
	...
	/**
	 * Purpose: To check if the schedule's total credits are within the credit
	 * range
	...
	private boolean withinCredits(Schedule schedule)
	{
		if ((schedule.getTotalCredits()) >= minCredits
				&& schedule.getTotalCredits() <= maxCredits)
		{
			return true;
		}
		else
	 ...
	 * (helper method for generateSchedules)
	 * 
	 * @param possibleSchedule The schedule being considered to be added to the
	 *                         top 3 schedules list
	 */
	private void addToTopThreeSchedules(Schedule possibleSchedule)
	{
		...
		else
		{
			...
			if (lowestSchedule != null && possibleSchedule
					.compareTo(lowestSchedule) == possibleSchedule)
			{
				topThreeSchedules
						.removeIf(schedule -> schedule == lowestSchedule);
				topThreeSchedules.add(possibleSchedule);
			}
		}
	 ...
	 * 
	 * @param schedule The schedule being generated that will have courses added
	 *                 to it
	 */
	private void addRandomCoursesToSchedule(Schedule schedule)
	{
		...
		// Try to add chosen sections until minimum credits are reached or
		// adding more would exceed max credits
		for (Course course : chosenSections)
		{
			...
			if (schedule.getTotalCredits() < minCredits)
			{
				if (schedule.getTotalCredits()
						+ course.getCourseCredits() <= maxCredits)
				{
	...
	/**
	 * Purpose: To cap the amount of schedules generated at the specified
	 * parameter amount
			...
			if (attemptsCopy % 25 == 0)
			{
				...
				if (schedulesDone > lastScheduleCount)
				{
					...
					else
					{
						...
						recentAvgTimePerSchedule = (recentAvgTimePerSchedule
								* 0.7) + (currentRate * 0.3);
					}
				...
				long remainingTime = (long) (recentAvgTimePerSchedule
						* remainingSchedules);
						...
						&& now - lastUiUpdateTime[0] > 700)
				{
					...
					SwingUtilities.invokeLater(() -> {
						loadingScreen.setProgress(currentSchedules,
								maxSchedules);
	...
	// Other Methods
	/**
	 * Purpose: To set the info for the min and max credits, break times,
	 * desired start and end times, desired campus location, and student's major
	 * distinction from the user data repository
	 * 
	 * @param newUserDataRepository          The user data repository to grab
	 *                                       the info from
	 * @param newScheduleGeneratorRepository The schedule generator repository
	 *                                       to set the schedule generator
	 *                                       repository instance variable to
	 */
	public void setScheduleGeneratorReposAndFileService(
			...
			ScheduleGeneratorRepository newScheduleGeneratorRepository)
	{
		...
		desiredStartAndEndTime = newUserDataRepository
				.getDesiredStartAndEndTime();
		desiredCampusLocation = newUserDataRepository
				.getDesiredCampusLocation();
		studentsMajorDistinction = newUserDataRepository
				.getStudentsMajorDistinction();
		scheduleGeneratorRepository = newScheduleGeneratorRepository;
	}
	 ...
	 * 
	 * @param completedCourses The list of completed courses to add to the
	 *                         schedule generator's course pool
	 */
	...
	/**
	 * Purpose: To create many possible schedule objects to compare to each
	 * other
	...
	public List<Schedule> generateTopThreeSchedules(int maxSchedules)
	{
		...
		List<Schedule> possibleSchedules = this
				.createPossibleSchedules(maxSchedules);
	...
	/**
	 * Purpose: To clear the course pool if the user selects back button on
	 * course info view
	 * 
	 */
	...
	/**
	 * Purpose: To check if a schedule with the same courses already exists in
	 * the list of schedules
	 * 
	 * @param list        The list of schedules to check against
	 * @param newSchedule The new schedule being considered for addition to the
	 *                    list
	 * @return true if a schedule with the same courses already exists, false
	 *         otherwise
	 */
